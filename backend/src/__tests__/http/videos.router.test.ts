...
describe("videos.router", () => {
  ...
  it("GET /domain/videos returns paginated results from the service", async () => {
    // GET /domain/videos should read `limit` and `offset` from the query
    // string, pass numeric values to the service, and return the service result
    // with status 200.
    const payload = {
      videos: [makeVideo()],
      total: 1,
  ...
  it("GET /domain/videos/:id returns a single video when the service finds one", async () => {
    // GET /domain/videos/:id should pass the path ID to the service and return
    // the found video with status 200.
  ...
  it("GET /domain/videos/:id returns 404 when the service returns null", async () => {
    // GET /domain/videos/:id should return a 404 API error with message
    // "Video not found" when the service returns null.
  ...
  it("POST /domain/videos validates the body and forwards parsed data to the service", async () => {
    // POST /domain/videos should accept a valid body, add the placeholder
    // uploadedByUserId, call the service with the final payload, and return
    // status 201 with the created video.
  ...
  it("POST /domain/videos rejects invalid payloads before the service is called", async () => {
    // POST /domain/videos should return 400 for an invalid request body and
    // should not call the create service.
  ...
  it("PUT /domain/videos/:id validates the body and forwards the id plus patch to the service", async () => {
    // PUT /domain/videos/:id should read the path ID, validate the update body,
    // call the service with the ID plus parsed patch, and return the updated
    // video.
  ...
  it("PUT /domain/videos/:id rejects invalid payloads before the service is called", async () => {
    // PUT /domain/videos/:id should return 400 for an invalid update body and
    // should not call the update service.
    const response = await request(app)
  ...
  it("DELETE /domain/videos/:id returns 204 after delegating to the service", async () => {
    // DELETE /domain/videos/:id should call the delete service with the path ID
    // and return an empty 204 response when deletion succeeds.
    const id = "22222222-2222-2222-2222-222222222222";
